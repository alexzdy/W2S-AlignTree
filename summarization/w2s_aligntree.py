...
class MCTSGenConfig:
    ...
    num_iterations: Optional[int] = 100  # Maximum number of iterations, m
    l: Optional[int] = 4  # history length
    entropy_weight: Optional[float] = 0.2  # Entropy weight for exploration, w
    eos_strings: Optional[list] = field(default_factory=lambda: ["\n"])
    others: GenConfig = field(default_factory=lambda: GenConfig(max_new_tokens=128, temperature=0.7, top_p=1.0, top_k=50))
...
class ScriptArguments:
    ...
    base_prompt_template:  Optional[str]  = "{raw_prompt}"
    dataset_name:          Optional[Text] = "/path/to/TLDR_new"  # The dataset organized according to the format in the appendix
...
for idx, raw_prompt in enumerate(tqdm.tqdm(dataset["raw_prompt"])):
    ...
    outputs = mcts_model.search(
        ...
        eos_strings=script_args.gen.eos_strings,
